...
let population = Array(num_gtypes.current).fill(0);
population[num_gtypes.current/2]=popsize.current;
let fitnesses = Array.from({length:num_gtypes.current}, () => getRandomNormal(1.0,fvar.current));
fitnesses[0]=1.5;
fitnesses[num_gtypes.current-1]=1.5;
...
function setup(){
    ...
    plot.getHistogram().setBgColors([color("#f2ea02")]);
    plot.getHistogram().setLineColors([color("#000000")]);
